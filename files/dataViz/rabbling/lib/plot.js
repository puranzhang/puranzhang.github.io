// d3.json("../data/wordBank_npf_tag.json", function(error, data) {
//     console.log(plotRegression(data, 31));
//     console.log(getProb(data, 'hamburger', 31));
//   });
...
function plotRegression(wordData) {
	...
	var xScale = d3.scaleLinear().domain([16, 30]).range([60, width-60]),
	    ...
	    modScale = d3.scaleLinear().domain(function(d) {return d3.extent()}).range([0,1]);
	...
	function plotWord(tempWord) {
	  ...
	  tempWord.values.forEach(function(d) {
	    ...
	    for (var i = 16; i < 31; i++) {
	      ...
	      //     .attr("r", 3)
	      //     .attr("fill", d3.hsl(word*350/len,100,100))
	    }
	    word += 1;
	  });
	  ...
	  var sample = [];
	  for(var i = 16; i < 31; i++) {
	    sample.push({"x":i, "y":result.equation[0] 
	                            + result.equation[1]*i 
	  ...
	  //console.log(func.attr("d"));
	  //return d3.select("#" + tempWord.key).attr("d");
	  return sample;
	}
